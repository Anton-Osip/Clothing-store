...
class Main {
    ...
    drowCartWindow(){
        $('.main').innerHTML='';
        const cart = [];
        this.productsData.forEach((product)=>{product.inCart==true?cart.push(product):'';})
        if(cart.length===0){
            $('.main').insertAdjacentHTML('beforeend', `
            <div class='container'>
                <div class = 'free'>
        ...
        document.querySelectorAll('.basket__delite').forEach((item)=>{item.addEventListener('click',this.deliteProduct.bind(this))})
    }
